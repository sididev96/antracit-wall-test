      ...
      SamModel.from_pretrained(modelId, {
        progress_callback: (progress: {
          ...
          if (progress.progress !== undefined) {
            console.log(
              `[SAM] Loading ${progress.file}: ${Math.round(
                progress.progress
              )}%`
            );
          }
...
export async function predictMask(
  ...
  console.log(
    `[SAM] Inference completed in ${(performance.now() - startTime).toFixed(
      0
    )}ms`
  );
  ...
  console.log(
    `[SAM] Best mask index: ${bestMaskIdx}, score: ${bestScore.toFixed(4)}`
  );
